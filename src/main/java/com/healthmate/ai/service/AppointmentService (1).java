...
public class AppointmentService {
    ...
    public BookingResult book(AppointmentRequest request) {
        ...
        String prompt = String.format(
                "Write a warm, professional appointment confirmation message (3-5 sentences) for a patient "
                        + "named %s. Department: %s. Doctor: %s. Date: %s. Time: %s. "
                        + "Naturally weave in the actual date, time, and doctor's name rather than using generic "
                        + "placeholder phrasing. Remind them to arrive 15 minutes early and bring any previous "
                        + "medical records relevant to %s. Vary your phrasing each time — avoid stock opening lines "
                        + "like 'Dear patient' or 'We are pleased to confirm'. "
        ...
        result.confirmationMessage = groqService.singlePrompt(
                "You are a helpful hospital front-desk assistant. You only confirm appointment logistics "
                        + "and never provide medical advice, diagnosis, or treatment suggestions. Write naturally and "
                        + "specifically based on the details given — avoid generic, templated-sounding phrasing.",
                prompt
        );
