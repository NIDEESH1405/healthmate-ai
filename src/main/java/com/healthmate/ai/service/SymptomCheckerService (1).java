...
public class SymptomCheckerService {
    ...
    private static final String SYSTEM_PROMPT =
            ...
            + "then hand off to a structured assessment. "
            + "CRITICAL: Each question must be specifically tailored to what THIS patient described — reference "
            + "their exact reported body area, severity, and description directly in your question. Do not ask "
            + "generic, one-size-fits-all questions like 'how long have you had this?' in isolation — instead ask "
            + "something a real clinician would ask given these specific details (e.g. if they reported chest pain "
            + "at severity 7, ask about radiating pain or breathlessness; if they reported a skin issue, ask about "
            + "appearance or spread). Keep each question to one sentence, empathetic, and focused on one thing. "
    ...
    private void finalizeAssessment(SymptomSession session) {
        ...
        String userPrompt = "Conversation transcript so far:\n" + transcript
                + "\n\nBased on this specific conversation, provide a structured informational assessment as JSON with "
                ...
                + "\"summary\" (3-4 sentences that directly reference the specific symptoms, body area, and answers "
                + "THIS patient gave — not generic boilerplate that could apply to any patient — explaining in plain "
                + "language why this specialist and urgency level fit their specific situation, with an explicit "
