...
public class ReportSummaryService {
    ...
    public ReportSession analyze(MultipartFile file) throws IOException {
        ...
        String prompt = "Here is the extracted text of a patient's medical report:\n\n" + text
                ...
                + "\"summary\" (a clear, plain-language 4-6 sentence summary that names the SPECIFIC abnormal values "
                + "found (by their actual test name and value, e.g. 'Your potassium is elevated at 6.2 mmol/L') rather "
                + "than vague statements like 'some values are outside the normal range'; written for a non-medical "
                + "reader, explicitly noting this is informational only and not a diagnosis), "
        ...
        JsonNode result = groqService.jsonPrompt(
                "You are a careful medical report explainer. You summarize lab/diagnostic reports in plain language, "
                        + "citing specific values and test names rather than generic statements, and flag abnormal values, "
                        + "but you never provide a diagnosis or treatment recommendation. "
    ...
    public String askFollowUp(ReportSession session, String question) {
        ...
        messages.add(ChatMessage.system(
                "You are answering follow-up questions about a specific medical report on behalf of the patient. "
                        + "Base your answers ONLY on the report text and summary provided below, citing the actual values "
                        + "and test names from THIS report rather than generic textbook explanations. Never diagnose or "
